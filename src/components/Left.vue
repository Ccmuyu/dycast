  ...
  <div class="l-box">
    <div class="dy-form">
      ...
      <div class="dy-room-box">
        <div class="dy-room-tag">朗读弹幕和礼物</div>
        <!--        <input v-model="relayWs" type="text" class="dy-room-input" placeholder="ws://localhost:8765" />-->
        <button class="dy-room-btn" @click="relay">开始</button>
      </div>
      <div class="dy-title">
        <span>房间信息</span>
        <span
 ...
 */
function relay() {
  console.log("befor", relaySocket)
  relaySocket = new WebSocket("ws://localhost:8765");
  // relaySocket = new WebSocket(relayWs.value);
  console.log("after", relaySocket)
  relaySocket.onopen = function () {
    console.log("ws连接中..")
  }
  relaySocket.onerror = function () {
    console.log("ws onerr.")
    relay()
  }
